...
void arrowToWASD(unsigned char& key) {
    ...
    if (inp == 0 || inp == 224) {
        int c = getch();
        if (c == RIGHT_ARROW) inp = 'u';
        else if (c == LEFT_ARROW) inp = 'i';
        else if (c == UP_ARROW) inp = 'o';
        else if (c == DOWN_ARROW) inp = 'p';
    }
    key = inp;
    return;
}
...
// ======== TEAM GAME (Alan) ========
void team(int uida, int uidb) {
    hideCursor();

    int rows = 8, cols = 8;
    // array[row][column]
    vector<vector<Cell>> boarda(rows, vector<Cell>(cols));
    vector<vector<bool>> moveablea(rows, vector<bool>(cols, false));
    string current_colora = TXT_BLUE;

    // array2
    vector<vector<Cell>> boardb(rows, vector<Cell>(cols));
    vector<vector<bool>> moveableb(rows, vector<bool>(cols, false));
    string current_colorb = TXT_BLUE;

    // vector<vector<Cell>> board2 = {
    //     {{false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}},
    //     {{false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}},
    //     {{false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}},
    //     {{false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}},
    //     {{false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}, {false, TXT_HIDDEN}},
    ...
    // creates a list of switchable pieces
    vector<ActivePiece> pieces_list;
    placeablePiecesa(board, pieces_list);

    // placeable 2
    vector<ActivePiece> pieces_list;
    placeablePiecesb(board, pieces_list);

    // skor 1
    int prev_scorea = 0;
    int scorea = prev_scorea;

    // skor 2
    int prev_scoreb = 0;
    int scoreb = prev_scoreb;
    // int combo = 0;
    ...
    while (true) {
        ...
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢    Score : 0                      ║      Score : 0                      ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║                                   |                                     ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢     W    : move piece up          ║       ↑    : move piece up          ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║     A    : move piece left        |       ←    : move piece left        ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢     S    : move piece down        ║       ↓    : move piece down        ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║     D    : move piece right       |       →    : move piece right       ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢    1-3   : select piece           ║      8-0   : select piece           ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║     A    : place piece            |       L    : place piece            ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢     S    : use item               ║       K    : use item               ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║    ESC   : exit game              |      ESC   : exit game              ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╟────┼────┼────┼────┼────┼────┼────┼────╢                                   ║                                     ╟────┼────┼────┼────┼────┼────┼────┼────╢
        // ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║                                   |                                     ║ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ │ ██ ║
        // ╚════╧════╧════╧════╧════╧════╧════╧════╝                                   ╧                                     ╚════╧════╧════╧════╧════╧════╧════╧════╝

        // score animation
        if (prev_scorea < scorea) {
            // increment the score
            int difference = scorea - prev_scorea;
            if (difference > 100) {
                prev_scorea += (scorea - prev_scorea) / 7;
            } else {
                prev_scorea += 7;
            }
        ...
        // score anim 2
        if (prev_scoreb < scoreb) {
            // increment the score
            int difference = scoreb - prev_scoreb;
            if (difference > 100) {
                prev_scoreb += (scoreb - prev_scoreb) / 7;
            } else {
                prev_scoreb += 7;
            }
            ...
            moveCursor(6, cols * 5 + 12); //move cursor ubah
            cout << prev_scoreb;
            this_thread::sleep_for(chrono::milliseconds(60));
        }
        ...
        // display teamDisplay
        if (update_display) {
            teamDisplay(boarda, moveablea, scorea, pieces_lista, boardb, moveableb, scoreb, pieces_listb);
            update_display = false;
            // cout << "display\n";
        }

        bool piece_placed[2] = {0, 0};

        // ask for input
        // arrowToUIOP, isMoveable, movePiece, isPlaceable, switchPiece, isSwitchable
        ...
        if (kbhit()) {
            ...
            // moves piece
            arrowToWASD(inp);
            if (isMoveable(moveablea, inp)) {
                movePiece(moveablea, inp);
            }

            // places piece
            if (inp == ' ' && isPlaceable(moveablea, board)) {
                const char* c_color = current_color.c_str();
                placePiece(moveable, board, c_color, pieces_list); //a dan b
                piece_placed[0] = true; //[0] , [1] array
            }
            ...
            score += line_count * 50;
            if (piece_placed && line_count > 0) {
                clearLines(board, clear_col, clear_row); //a dan b
                moveCursor(6, cols * 5 + 12);
                cout << printPadding(static_cast<int>(log10(score) + 2), ' ');
                // update_score = true;
            }
